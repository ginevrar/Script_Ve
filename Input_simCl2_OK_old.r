...
abline(h=36)
ind2[120];cit[120,];plot(years, ind_mehg, type='l')
...
barplot(tutt, beside=F, width = 10,legend.text=F,space=0,
...
perc_dep<- tott[2]/tott[7]*100
perc_riv<- (tott[3]+tott[4])/tott[7]*100
perc_cit<- tott[5]/tott[7]*100
perc_ind<- (tott[6]/tott[7]*100)

perc_dep_media<-tapply(perc_dep[2:201,], rep(1:(length(perc_dep[2:201,])/20),each = 20),mean)
perc_riv_media<-tapply(perc_riv[2:201,], rep(1:(length(perc_riv[2:201,])/20),each = 20),mean)
perc_cit_media<-tapply(perc_cit[2:201,], rep(1:(length(perc_cit[2:201,])/20),each = 20),mean)
perc_ind_media<-tapply(perc_ind[2:201,], rep(1:(length(perc_ind[2:201,])/20),each = 20),mean)


perctutt2<-matrix(as.numeric(perc_dep_media),as.numeric(perc_riv_media),
                      as.numeric(perc_cit_media),as.numeric(perc_ind_media))
perctutt2<-matrix(perctutt2);
perctutt2<-t(perctutt2)

barplot(perctutt2, beside=F, width = 10,legend.text=F,ylim=c(0,110),space=0,
        border = NA, col=c('cyan3','royalblue','orange',
                           'grey40'),
        main='Hg inputs to the Venice Lagoon')
...
barplot(perctutt, beside=F, width = 10,legend.text=F,ylim=c(0,110),space=0,
        border = NA, col=c('cyan3','royalblue','orange',
                           'grey40'),
        main='Hg inputs to the Venice Lagoon')
mtext(text=expression(paste('%')),side=2,  line=2.5, cex=1.3)
