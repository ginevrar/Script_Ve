...
de2<-depoo$dep_zeroEm[1:201]  
de3<-de2[1:201]

  plot(de, type='l', ylim=c(0,80))
  par(new=T)
  plot(de2, type='l',col=3, ylim=c(0,80))
  par(new=T)
  plot(de3, type='l',col=5, ylim=c(0,80))
str(de)
tail(de3)
...
par(new=T)
plot(de3)
de_s2<-depoo$dep_zeroEm
de_s3<-depoo$Hg_control
de_s4<-depoo$Hg_const
de_s5<-depoo$A1B1

plot(depoo$dep_zeroEm,ylim=c(0,20),type='l')
par(new=T)
plot(depoo$atm_dep_Cl,ylim=c(0,20),type='l', col=2)
...
boxplot(r, ylim=c(1,50), col='grey80', ylab='', xlab='', 
        boxwex=1.2, main='Atmospheric deposition rate')
text(1,18,'range for marine areas \n from GEOS-Chem model (UNEP, 2013)', cex=1.2)
par(new=T)
plot(years,de2_g_km2_y, type='l', ylim=c(1,50),lwd=3,lty=2, col='cyan4', xlab='year',ylab='g km-2 y-1')  ## range 2 - 25 g km-2 y-1 over marine areas (UNEP, 2013)
par(new=T)
plot(years,de3_g_km2_y, type='l', ylim=c(1,50),lwd=3, lty=3,col='cyan4', xlab='year',ylab='g km-2 y-1')  ## range 2 - 25 g km-2 y-1 over marine areas (UNEP, 2013)
par(new=T)
plot(years,de4_g_km2_y, type='l', ylim=c(1,50),lwd=3,lty=4, col='cyan4', xlab='year',ylab='g km-2 y-1')  ## range 2 - 25 g km-2 y-1 over marine areas (UNEP, 2013)
par(new=T)
plot(years,de5_g_km2_y, type='l', ylim=c(1,50),lwd=3,lty=5, col='cyan4', xlab='year',ylab='g km-2 y-1')  ## range 2 - 25 g km-2 y-1 over marine areas (UNEP, 2013)
par(new=T)
plot(years,dep_g_km2_y, type='l', ylim=c(1,50),lwd=3, col='cyan3', xlab='year',ylab='g km-2 y-1')  ## range 2 - 25 g km-2 y-1 over marine areas (UNEP, 2013)
text(18,5,'input for \n the Venice lagoon', cex=1.2)
text(ladata[30],26,'D', cex=2.5)

dev.off()

write.table(all_input,file='all_input_hgII_noSeason_ZeroEm.txt')
