...
#fraction of POM
fPOM<-POMs/TOTs; fsilt <-silts/TOTs;POM_perc<-POMs/TOTs*100
...
POM_res1_m_day<-POM_res$sn1; silt_res1_m_day<-silt_res$sn1 # [m d-1]
...
head(POMs$sc6,30); 
head(POM_res6_m_day,30); 
plot(head(SEDhg$sc6,530))
plot(head(SEDhg$sc6*POMs$sc6*fhgPOM,530))

plot(head(SEDhg$sc6*silts$sc6*fhgsilt/1000,530)+head(SEDhg$sc6*POMs$sc6*fhgPOM/1000,530))
plot(head(Phgs$sc6,530))

plot(head(SEDhg$sc6*TOTs$sc6,530))

head(Phgs$sc6,30); 



head(silts$ss10*fhgsilt*SEDhg$ss10/1000)
...
bb<-
Net_kgd<-(Tot_Depo_hg_ugm2d-(Tot_res_hg_ugm2d))*area/10^9
# Ok netto 

par(mfrow=c(1,2))
plot(rdate,Net_kgd*365, col='#E28DA8', type='l', lwd=3)
abline(h=0)
plot((Tot_Depo_hg_kgy-Tot_res_hg_kgy),col='#ab2b53',type='l', lwd=3)

plot(rdate,cumsum(Net_kgd*365), col='#B9FF73',type='l', lwd=3)

str(Net_kgd)

Net_kg_d_mediaaaa<-tapply(Net_kgd[2:73768], rep(1:(length(Net_kgd[2:73768])/367), each = 367),mean)
Net_kg_d_somma<-tapply(Net_kgd[2:73768], rep(1:(length(Net_kgd[2:73768])/367), each = 367),sum)

plot(Net_kg_d_mediaaaa*365, type='l', col=1, ylim=c(0,2100000))
par(new=T)
plot(Net_kg_d_somma, type='l', col=2, ylim=c(0,2100000))

plot(cumsum(Net_kg_d_somma*365), col='#B9FF73',type='l', lwd=3)

Marghera_kg_y_media<-tapply(Marghera,rep(1:(length(Marghera)/12),each = 12),mean)
...
plot(Tot_Depo_hg_kgy[1191:1214], type='l')
par(new=t)
plot(Tot_res_hg_kgy[1191:1214], type='l')

win.graph()
plot(rdate,Tot_Depo_hg_kgy, col='blue',type='l')
par(new=T)
plot(rdate,-Tot_res_hg_kgy, col='orange',type='l')
par(new=T)
